...
impl ColwiseRegBlockAdjustTracker {
    ...
    pub fn adjust_block_shape(&self) -> [usize; 2] {
        // Regular colwise block adjust scheme now sets a regular block size.
        let block_shape = [8 as usize; 2];
        return block_shape;
    }

    pub fn adjust_window_shape(&mut self, block_token: usize, block_anchor: [usize; 2],
        ...
        if n1_tk_acr.is_none() {
            let window_shape = [self.lane_num, 1];
        ...
        if n2_tk_acr.is_none() {
            let window_shape = [self.lane_num / 2, 2];
            self.window_shape.insert(block_token, window_shape);
            return window_shape;
        }
        ...
        let win_h = if (n1_cost as f32 / n1_ele_size as f32)
