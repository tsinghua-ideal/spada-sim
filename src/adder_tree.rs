...
impl Multiplier {
    ...
    pub fn multiply(&mut self) {
        if self.a_eles.is_none() || self.b.is_none() {
            ...
            match self.a_eles.as_ref().unwrap().iter().find(|a| a.idx[1] == b.idx[0]) {
                Some(a) => {
...
impl MergeTree {
    ...
    pub fn push_element(&mut self, leaf_idx: usize, element: Option<Element>) {
        if element.is_some() {
            ...
            assert!(
                mem::replace(
                    &mut self.merge_tree[self.tree_depth][leaf_idx],
                    element
                ).is_none(),
                "Node at {} already has {:?}",
                leaf_idx,
                self.merge_tree[self.tree_depth][leaf_idx].as_ref().unwrap().idx
            );
        }
    }
    ...
    pub fn compare_pop(&mut self, lvl: usize, left_node: usize, right_node: usize) -> Option<Element> {
        let le = & self.merge_tree[lvl][left_node];
        let re = & self.merge_tree[lvl][right_node];
        if re.is_none() || le.is_some() && le.as_ref().unwrap().idx[1] <= re.as_ref().unwrap().idx[1] {
    ...
    pub fn idle(&self) -> bool {
        for layer in self.merge_tree.iter() {
            if layer.iter().any(|n| n.is_some() && n.as_ref().unwrap().idx != [usize::MAX; 2]) {
                return false;
            }
        }
    ...
    pub fn print_merge_tree(&self) {
        ...
        for lvl in 0..self.tree_depth+1 {
            trace_print!("{:?} ", self.merge_tree[lvl].iter().map(|e| e.as_ref().map(|_e| _e.idx)).collect::<Vec<Option<[usize; 2]>>>());
        }
        trace_println!("");
        for lvl in 0..self.tree_depth+1 {
...
impl AdderTree {
    ...
    pub fn idle(&self) -> bool {
        let is_idle = self.merge_tree.idle()
            && self.multiplier.idle()
            && self.adder.idle();
        return is_idle;
    }
    ...
    pub fn set_task(&mut self, task: Option<(usize, Task)>) -> usize {
        if task.is_none() {
            ...
            self.multiplier.set_as(
                Some(task.a_eles.iter().filter_map(|e| e.clone()).collect())
            );
            self.mem_finish_cycle = None;
            return a_latency;
        }
